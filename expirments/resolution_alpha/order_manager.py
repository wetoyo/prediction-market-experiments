...
class OrderManager:
    # Class-level fallback so an instance built via OrderManager.__new__
    # (bypassing __init__ -- see tests/test_order_manager.py's
    # TestPlaceOrderWiring, which injects a fake self._client directly) still
    # has a _state attribute, rather than raising AttributeError the first
    # time buy_favored_side tries to record to it.
    _state: KalshiStateManager | None = None

    def __init__(self, dry_run: bool = DRY_RUN, state: KalshiStateManager | None = None):
        self.dry_run = dry_run
        # KalshiStateManager centralizes what breaks when more than one
        # experiment shares this Kalshi account at once: pacing every
        # experiment's API calls into one system-wide request schedule, and
        # (once configured, see config.py's CAPITAL_FRACTION/CAPITAL_CAP_DOLLARS)
        # capping how much of the real balance this experiment's Kelly sizing
        # is allowed to treat as its own. See
        # ../../prediction_market_scraper/Clients/Kalshi/README.md.
    ...
    def get_balance_dollars(self) -> float:
        ...
        to no-op, so this returns the full real balance unless that's been
        explicitly configured down), for Kelly sizing (runner.py's
        ...
        """
        total = float(self._state.get_balance()["balance_dollars"])
