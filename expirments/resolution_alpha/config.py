...
EXPERIMENT_NAME = "resolution_alpha"
...
# trading this account live at the same time -- e.g. running resolution_alpha
# and btc_implied_prob live together, each at 1.0 (the default), means both
# independently size as if they alone own the full balance, and the combined
# intended stake across both can run past what either's own KELLY_FRACTION
# was tuned to risk. Set below 1.0 (e.g. 0.5 each, for two experiments) to
# partition the account instead. See
# ../../prediction_market_scraper/Clients/Kalshi/README.md's "Capital
# allocation" section before changing this on the live system.
CAPITAL_FRACTION = _float_env("RESOLUTION_ALPHA_CAPITAL_FRACTION", 1.0)
...
_capital_cap_dollars_env = os.environ.get("RESOLUTION_ALPHA_CAPITAL_CAP_DOLLARS")
...
#
# Restored to the pre-bundle 3.0 on 2026-08-30: the whole 2026-08-30 calibration
# bundle was neutralized (see SIGMA_SAFETY_FACTOR etc. below) after it took the
# runner to zero entries. This is the original fill-execution guard, not a bundle
# knob; 3.0 is where it sat before any of this. Raise via
# RESOLUTION_ALPHA_MIN_ENTRY_SECONDS_LEFT once the bundle is re-tuned.
...
# the original 1.25.
#
# Set to 1.0 (identity, no-op) on 2026-08-30. The knob and its probability.py
# wiring stay in place, but 1.4 / 1.6 / the diffusion floor / the distance gate
# between them starved the runner to zero entries and progressive walkbacks never
# recovered it. Reverted the whole bundle to inert; the correct SIGMA_SAFETY_FACTOR
# (and TAIL_SIGMA_DIFFUSION_FLOOR_FRAC / MODEL_PROB_CAP / MIN_STRIKE_DISTANCE_FRAC)
# is to be re-derived from fresh data. Restore via RESOLUTION_ALPHA_SIGMA_SAFETY_FACTOR
# or by editing this default. 1.0 => sigma_used * 1.0, unchanged.
SIGMA_SAFETY_FACTOR = _float_env("RESOLUTION_ALPHA_SIGMA_SAFETY_FACTOR", 1.0)
...
# formula back.
#
# Set to 0.0 (disabled, no-op) on 2026-08-30 -- part of neutralizing the whole
# 2026-08-30 bundle (see SIGMA_SAFETY_FACTOR). The floor logic stays wired in
# probability.py; 0.0 => diffusion_floor 0.0 => max(sigma_used, 0.0) unchanged.
# The near-strike overconfidence this was written for is real and still needs a
# fix -- re-derive the fraction from fresh data.
TAIL_SIGMA_DIFFUSION_FLOOR_FRAC = _float_env("RESOLUTION_ALPHA_TAIL_SIGMA_DIFFUSION_FLOOR_FRAC", 0.0)
...
# Stays at 0.0 as part of neutralizing the full 2026-08-30 bundle (the runner.py
# near_strike gate + skip stat remain wired; frac < 0.0 is never true, so inert).
...
#
# Set to 1.0 (disabled, no-op) on 2026-08-30 -- part of neutralizing the full
# 2026-08-30 bundle (see SIGMA_SAFETY_FACTOR). The clamp stays wired in
# probability.py; 1.0 => min(max(prob_yes, 0.0), 1.0), unchanged for any valid
# probability. Re-derive the real cap from fresh calibration data.
MODEL_PROB_CAP = _float_env("RESOLUTION_ALPHA_MODEL_PROB_CAP", 1.0)
...
#
# Set to 0.0 (no-op) on 2026-08-30 -- part of neutralizing the full 2026-08-30
# bundle (see SIGMA_SAFETY_FACTOR). The flip_needs_confirm branch stays wired in
# _check_exit_conditions; 0.0 => `seconds_left < 0.0` is never true, so a flip
# exit fires immediately again. Restore via RESOLUTION_ALPHA_FLIP_EXIT_CONFIRM_SECONDS.
FLIP_EXIT_CONFIRM_SECONDS = _float_env("RESOLUTION_ALPHA_FLIP_EXIT_CONFIRM_SECONDS", 0.0)
